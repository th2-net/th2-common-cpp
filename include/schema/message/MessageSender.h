...
public:
    ...
    virtual void init(connection_manager_ptr connection_manager,
                      ...
                      const std::string_view send_queue) = 0;

    virtual void send(const MessageBatchType& message) = 0;
};

template <class MessageBatchType>
using message_sender_ptr = std::shared_ptr<MessageSender<MessageBatchType>>;
